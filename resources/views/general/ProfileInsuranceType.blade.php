                ...
                <div class="page-header">
                    <nav class="breadcrumb-one" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>กำหนดประเภทประกันภัย</span></li>
                        </ol>
                    </nav>
                </div>
            </li>
                            ...
                            <div id="mailbox-inbox" class="accordion mailbox-inbox p-3">
                                <div class="form-group row mt-4">
                                    <div class="col-2 mt-1">
                                        <button type="button" class="btn btn-outline-primary" id="Add">เพิ่มประเภทประกันภัย</button>
                                    </div>
                                </div>
                                <div class="table-responsive" style="height: 650px;">
                                    <table class="table table-bordered mb-4" >
                                        <thead style="background: #fa4af1">
                                            <tr>
                                        ...
                                        <tbody>
                                            @foreach ($insurerType as $item)
                                            <tr>
                                                <td>{{ $item->InsureTypeCode }}</td>
                                                <td>{{ $item->InsureTypeName }}</td>
                                                <td>{{ $item->InsureTypeRemark }}</td>
                                                <td class="text-center">
                                                    <ul class="table-controls">
                                                        <li class="edit" data-id="{{ $item->InsureTypeCode }}">
                                                        ...
                                                        <li class="delete" data-id="{{ $item->InsureTypeCode }}"> 
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">เพิ่มประเภทประกันภัย</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                </button>
            </div>
            <div class="modal-body" style="height: 200px;"> 
                <form id="SaveInsurerType" action="javascript:void(0);" >
                    <div class="form-row">
                        <div class="col-md-3 mb-4">
                            <label>รหัสประเภทประกันภัย</label>
                            <input type="text" class="form-control required"  name="insurertypecode" placeholder="รหัสประเภทประกันภัย" >
                        </div>
                        <div class="col-md-5 mb-4">
                            <label>ชื่อประเภทประกันภัย</label>
                            <input type="text" class="form-control required" name="insurertypename" placeholder="ชื่อประเภทประกันภัย" >
                        </div>
                        <div class="col-md-4 mb-4">
                            <label>หมายเหตุ</label>
                            <input type="text" class="form-control " name="insurertyperemark" placeholder="หมายเหตุ" >
                        </div>
...
<script>
    ...
    $('#Add').click(function (e) { 
        ...
        $('#AddGroup').modal('show');
        $('#type').val('0');
        $("input[name='insurertypecode']").attr('readonly',false);
        $("#SaveInsurerType").trigger('reset');
    });
    ...
    $('.edit').click(function(e){
        ...
        $.ajax({
            type: "post",
            url: url+"/ProfileInsuranceTypeGet",
            data: {'id' : id},
            // dataType: "dataType",
            success: function (response) {
                $("input[name='insurertypecode']").val(response.InsureTypeCode);
                $("input[name='insurertypecode']").attr('readonly',true);
                $("input[name='insurertypename']").val(response.InsureTypeName);
                $("input[name='insurertyperemark']").val(response.InsureTypeRemark);
                $('#type').val('1');
                $('#AddGroup').modal('show');
            }
        });
    ...
    $('.delete').click(function(e){
        ...
        swal({
            ...
            if (result.value) {
                $.ajax({
                    type: "post",
                    url: url+"/ProfileInsuranceTypeDelete",
                    data: {'id' : id},
                    // dataType: "dataType",
                    success: function (response) {
                        if(response == 'success'){
    ...
    $('#SaveInsurerType').submit(function (e) { 
        e.preventDefault();
        ...
        if(required_status){
            $.ajax({
                type: "post",
                url: url+"/ProfileInsuranceTypeSave",
                data: $(this).serialize(),
                // dataType: "dataType",
                success: function (response) {
                    if(response == 'success'){
