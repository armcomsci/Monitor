                ...
                <div class="page-header">
                    <nav class="breadcrumb-one" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>กำหนดประเภทการหัก</span></li>
                        </ol>
                    </nav>
                </div>
            </li>
                            ...
                            <div id="mailbox-inbox" class="accordion mailbox-inbox p-3">
                                <form id="FormAddGroup" onSubmit="return false">
                                    <div class="form-group row mt-4">
                                        <div class="col-2 mt-1">
                                            <button type="button" class="btn btn-outline-primary" id="Add">เพิ่มกำหนดประเภทการหัก</button>
                                        </div>
                                    </div>
                                </form>
                                <div class="table-responsive" style="height: 650px;">
                                    <table class="table table-bordered mb-4" >
                                        ...
                                        <tbody>
                                            @foreach ($DedType as $item)
                                            <tr>
                                                <td>{{ $item->DedTypeDesc }}</td>
                                                <td>
                                                    @php
                                                        $CheckDedType = "";
                                                        if($item->DedTypeUse == "Y"){
                                                            $CheckDedType = "Checked";
                                                        }
                                                    ...
                                                    <label class="switch s-icons s-outline  s-outline-success  mb-4 mr-2">
                                                        <input type="checkbox" {{ $CheckDedType }} class="change_st_flag"  data-id="{{ $item->DedTypeID }}">
                                                        <span class="slider"></span>
                                                    </label>
                                                </td>
                                                <td>
                                                    <label class="switch s-icons s-outline  s-outline-success  mb-4 mr-2">
                                                        <input type="checkbox" {{ $CheckCalVat }} class="change_st_vat"  data-id="{{ $item->DedTypeID }}">
                                                        <span class="slider"></span>
                                                    </label>
                                                </td>
                                                <td class="text-center">
                                                    <ul class="table-controls">
                                                        <li class="edit" data-id="{{ $item->DedTypeID }}">
                                                        ...
                                                        <li class="delete" data-id="{{ $item->DedTypeID }}">
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">เพิ่มกำหนดประเภทการหัก</h5>
                ...
                <form id="SaveInsurer" action="javascript:void(0);" >
                    <div class="form-row">
                        <div class="col-md-12 mb-4">
                            ...
                            <input type="text" class="form-control required" name="DedTypeDesc" placeholder="ชื่อประเภท" >
                        </div>
...
<script>
    ...
    $('.edit').click(function(e){
        ...
        $.ajax({
            type: "post",
            url: url+"/ProfileFundGet",
            data: {'id' : id},
            // dataType: "dataType",
            success: function (response) {
                $("input[name='DedTypeDesc']").val(response.DedTypeDesc);
                $('#type').val(id);
                $('#AddGroup').modal('show');
            }
        });
