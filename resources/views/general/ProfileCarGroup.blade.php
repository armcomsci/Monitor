                ...
                <div class="page-header">
                    <nav class="breadcrumb-one" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>เพิ่มรุ่นรถ</span></li>
                        </ol>
                    </nav>
                </div>
            </li>
                                ...
                                <form id="FormAddGroup" onSubmit="return false">
                                    <div class="form-group row mt-4">
                                        <div class="col-2 mt-1">
                                            <button type="button" class="btn btn-outline-primary" id="Add">เพิ่มรุ่นรถ</button>
                                        </div>
                                    </div>
                                </form>
                                    ...
                                    <table class="table table-bordered mb-4" >
                                        ...
                                        <tbody>
                                            @foreach ($CarGroup as $item)
                                                <tr>
                                                    <td>{{ $item->CarSerieCode }}</td>
                                                    <td>{{ $item->CarSerieName }}</td>
                                                    <td>{{ $item->CarBrandName }}</td>
                                                    <td>{{ $item->CarSerieRemark }}</td>
                                                    <td class="text-center">
                                                        <ul class="table-controls">
                                                            <li class="edit" data-id="{{ $item->CarSerieCode }}">
                                                            ...
                                                            <li class="delete" data-id="{{ $item->CarSerieCode }}">
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">เพิ่มรุ่นรถ</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                </button>
            </div>
            <div class="modal-body" style="height: 300px;"> 
                <form id="SaveCarGroup" action="javascript:void(0);" >
                    <div class="form-row">
                        <div class="col-md-3 mb-4">
                            <label>รหัสรุ่นรถ</label>
                            <input type="text" class="form-control required"  name="carseriecode" placeholder="รหัสรุ่นรถ" >
                        </div>
                        <div class="col-md-5 mb-4">
                            <label>ชื่อรุ่นรถ</label>
                            <input type="text" class="form-control required" name="carseriename" placeholder="ชื่อรุ่นรถ" >
                        </div>
                        <div class="col-md-4 mb-4">
                            <label>ยี่ห้อรถ</label>
                            <select class="form-control required" name="carbrandcode">
                                @php
                                    $CarBand = GetCarBrand();
                                @endphp
                                <option value=""></option>
                                @foreach ($CarBand as $car)
                                    <option value="{{ $car->CarBrandCode }}">{{ $car->CarBrandName }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-12 mb-4">
                            <label>หมายเหตุ</label>
                            <input type="text" class="form-control " name="carserieremark" placeholder="หมายเหตุ" >
                        </div>
...
<script>
    ...
    $('#Add').click(function (e) { 
        ...
        $('#AddGroup').modal('show');
        $('#type').val('0');
        $("input[name='carseriecode']").attr('readonly',false);
        $("#SaveCarGroup").trigger('reset');
    });
    ...
    $('.edit').click(function(e){
        ...
        $.ajax({
            type: "post",
            url: url+"/ProfileCarGroupGet",
            data: {'id' : id},
            // dataType: "dataType",
            success: function (response) {
                $("input[name='carseriecode']").val(response.CarSerieCode);
                $("input[name='carseriecode']").attr('readonly',true);
                $("input[name='carseriename']").val(response.CarSerieName);
                $("select[name='carbrandcode']").val(response.CarBrandCode);
                $("input[name='carserieremark']").val(response.CarSerieRemark);
                $('#type').val('1');
                $('#AddGroup').modal('show');
            }
        });
    ...
    $('#SaveCarGroup').submit(function (e) { 
        e.preventDefault();
