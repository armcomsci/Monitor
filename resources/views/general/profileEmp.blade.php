                ...
                <div class="page-header">
                    <nav class="breadcrumb-one" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>เพิ่มกลุ่มพนักงานใหม่</span></li>
                        </ol>
                    </nav>
                </div>
            </li>
                            ...
                            <div id="mailbox-inbox" class="accordion mailbox-inbox p-3">
                                <form id="FormAddGroup" onSubmit="return false">
                                    <div class="form-group row mt-4">
                                        <div class="col-2 mt-1">
                                            <button type="button" class="btn btn-outline-primary" id="Add">เพิ่มกลุ่มพนักงานใหม่</button>
                                        </div>
                                    </div>
                                </form>
                                <div class="table-responsive" style="height: 650px;">
                                    <table class="table table-bordered mb-4" >
                                        ...
                                        <tbody>
                                            @foreach ($lmEmp as $item)
                                                <tr>
                                                    <td>{{ $item->EmpGroupCode }}</td>
                                                    <td>{{ $item->EmpGroupName }}</td>
                                                    <td>{{ $item->Remark }}</td>
                                                    <td class="text-center">
                                                        <ul class="table-controls">
                                                            <li class="edit" data-id="{{ $item->EmpGroupCode }}">
                                                            ...
                                                            <li class="delete" data-id="{{ $item->EmpGroupCode }}">
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">เพิ่มกลุ่มพนักงานใหม่</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                </button>
            </div>
            <div class="modal-body" style="height: 210px;"> 
                <form id="SaveEmpGroup" action="javascript:void(0);" >
                    <div class="form-row">
                        <div class="col-md-3 mb-4">
                            <label>รหัสกลุ่มพนักงาน</label>
                            <input type="text" class="form-control required"  name="groupcode" placeholder="รหัสกลุ่มพนักงาน" >
                        </div>
                        <div class="col-md-5 mb-4">
                            <label>ชื่อกลุ่มพนักงาน</label>
                            <input type="text" class="form-control required" name="groupname" placeholder="ชื่อกลุ่มพนักงาน" >
                        </div>
                        <div class="col-md-4 mb-4">
                            <label>หมายเหตุ</label>
                            <input type="text" class="form-control " name="groupremark" placeholder="หมายเหตุ" >
                        </div>
...
<script>
    ...
    $('#Add').click(function (e) { 
        ...
        $('#AddGroup').modal('show');
        $('#type').val('0');
        $("input[name='groupcode']").attr('readonly',false);
        $("#SaveEmpGroup").trigger('reset');
    });
    ...
    $('.edit').click(function(e){
        ...
        $.ajax({
            type: "post",
            url: url+"/ProfileEmpGet",
            data: {'id' : id},
            // dataType: "dataType",
            success: function (response) {
                $("input[name='groupcode']").val(response.EmpGroupCode);
                $("input[name='groupcode']").attr('readonly',true);
                $("input[name='groupname']").val(response.EmpGroupName);
                $("input[name='groupremark']").val(response.Remark);
                $('#type').val('1');
                $('#AddGroup').modal('show');
            }
        });
    ...
    $('#SaveEmpGroup').submit(function (e) { 
        e.preventDefault();
