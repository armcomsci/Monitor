                                ...
                                <form id="FormAddGroup" onSubmit="return false">
                                    <div class="form-group row mt-4">
                                        <div class="col-2 mt-1">
                                            <button type="button" class="btn btn-outline-primary" id="Add">เพิ่มรหัสบริษัทประกันภัย</button>
                                        </div>
                                    </div>
                                </form>
                                <div class="table-responsive" style="height: 650px;">
                                    <table class="table table-bordered mb-4" >
                                        <thead style="background: #5c4afa">
                                            <tr>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{ $item->InsureCompCode }}</td>
                                                <td>{{ $item->InsureCompName }}</td>
                                                <td>{{ $item->InsureCompAddr }}</td>
                                                <td>{{ $item->InsureCompTel }}</td>
                                                <td>{{ $item->InsureCompFax  }}</td>
                                                <td>{{ $item->InsureCompRemark }}</td>
                                                <td class="text-center">
                                                    <ul class="table-controls">
                                                        <li class="edit" data-id="{{ $item->InsureCompCode }}">
                                                        ...
                                                        <li class="delete" data-id="{{ $item->InsureCompCode }}">
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">เพิ่มรหัสบริษัทประกันภัย</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                </button>
            </div>
            <div class="modal-body" style="height: 400px;"> 
                <form id="SaveInsurer" action="javascript:void(0);" >
                    <div class="form-row">
                        <div class="col-md-3 mb-4">
                            <label>รหัสบริษัทประกันภัย</label>
                            <input type="text" class="form-control required"  name="insurercode" placeholder="รหัสบริษัทประกันภัย" >
                        </div>
                        <div class="col-md-5 mb-4">
                            <label>ชื่อบริษัทประกันภัย</label>
                            <input type="text" class="form-control required" name="insurername" placeholder="ชื่อบริษัทประกันภัย" >
                        </div>
                        <div class="col-md-4 mb-4">
                            <label>โทรศัพท์</label>
                            <input type="text" class="form-control required" name="insurertel" placeholder="โทรศัพท์" >
                        </div>
                        <div class="col-md-8 mb-4">
                            <label>ที่อยู่</label>
                            <input type="text" class="form-control required" name="insureraddr" placeholder="ที่อยู่" >
                        </div>
                        <div class="col-md-4 mb-4">
                            <label>แฟ็กซ์</label>
                            <input type="text" class="form-control " name="insurerfax" placeholder="แฟ็กซ์" >
                        </div>
                        <div class="col-md-12 mb-4">
                            <label>หมายเหตุ</label>
                            <input type="text" class="form-control " name="insurerremark" placeholder="หมายเหตุ" >
                        </div>
...
<script>
    ...
    $('#Add').click(function (e) { 
        ...
        $('#AddGroup').modal('show');
        $('#type').val('0');
        $("input[name='insurercode']").attr('readonly',false);
        $("#SaveInsurer").trigger('reset');
    });
    ...
    $('.edit').click(function(e){
        ...
        $.ajax({
            type: "post",
            url: url+"/ProfileInsurerGet",
            data: {'id' : id},
            // dataType: "dataType",
            success: function (response) {
                $("input[name='insurercode']").val(response.InsureCompCode);
                $("input[name='insurercode']").attr('readonly',true);
                $("input[name='insurername']").val(response.InsureCompName);
                $("input[name='insurertel']").val(response.InsureCompTel);
                $("input[name='insureraddr']").val(response.InsureCompAddr);
                $("input[name='insurerfax']").val(response.InsureCompFax);
                $("input[name='insurerremark']").val(response.InsureCompRemark);
                $('#type').val('1');
                $('#AddGroup').modal('show');
            }
        });
    ...
    $('.delete').click(function(e){
        ...
        swal({
            ...
            if (result.value) {
                $.ajax({
                    type: "post",
                    url: url+"/ProfileInsuranceDelete",
                    data: {'id' : id},
                    // dataType: "dataType",
                    success: function (response) {
                        if(response == 'success'){
