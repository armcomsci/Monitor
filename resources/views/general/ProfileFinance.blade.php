                ...
                <div class="page-header">
                    <nav class="breadcrumb-one" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>กำหนดรหัสบริษัทการเงิน</span></li>
                        </ol>
                    </nav>
                </div>
            </li>
                            ...
                            <div id="mailbox-inbox" class="accordion mailbox-inbox p-3">
                                <div class="form-group row mt-4">
                                    <div class="col-2 mt-1">
                                        <button type="button" class="btn btn-outline-primary" id="Add">เพิ่มรหัสบริษัทการเงิน</button>
                                    </div>
                                </div>
                                <div class="table-responsive" style="height: 650px;">
                                    <table class="table table-bordered mb-4" >
                                        <thead style="background: #f89c33">
                                            <tr>
                                            <th>รหัสบริษัทการเงิน</th>
                                            <th>ชื่อบริษัทการเงิน</th>
                                            <th>ที่อยู่</th>
                                            <th>โทรศัพท์</th>
                                            <th>แฟ็กซ์</th>
                                            <th>หมายเหตุ</th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ $item->FinanceAddr }}</td>
                                                <td>{{ $item->FinanceTel }}</td>
                                                <td>{{ $item->FinanceFax }}</td>
                                                <td>{{ $item->FinanceRemark }}</td>
                                                <td class="text-center">
                                                    <ul class="table-controls">
                                                        <li class="edit" data-id="{{ $item->FinanceCode }}">
                                                        ...
                                                        <li class="delete" data-id="{{ $item->FinanceCode }}">
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">เพิ่มรหัสบริษัทการเงิน</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                </button>
            </div>
            <div class="modal-body" style="height: 390px;"> 
                <form id="SaveFinnance" action="javascript:void(0);" >
                    <div class="form-row">
                        <div class="col-md-3 mb-4">
                            <label>รหัสบริษัทการเงิน</label>
                            <input type="text" class="form-control required"  name="financecode" placeholder="รหัสบริษัทการเงิน" >
                        </div>
                        <div class="col-md-5 mb-4">
                            <label>ชื่อบริษัทการเงิน</label>
                            <input type="text" class="form-control required" name="financename" placeholder="ชื่อบริษัทการเงิน" >
                        </div>
                        <div class="col-md-4 mb-4">
                            <label>โทรศัพท์</label>
                            <input type="text" class="form-control required" name="financetel" placeholder="โทรศัพท์" >
                        </div>
                        <div class="col-md-12 mb-4">
                            <label>ที่อยู่</label>
                            <input type="text" class="form-control required" name="financeaddr" placeholder="ที่อยู่" >
                        </div>
                        <div class="col-md-12 mb-4">
                            <label>หมายเหตุ</label>
                            <input type="text" class="form-control " name="financeremark" placeholder="หมายเหตุ" >
                        </div>
...
<script>
    ...
    $('#Add').click(function (e) { 
        ...
        $('#AddGroup').modal('show');
        $('#type').val('0');
        $("input[name='financecode']").attr('readonly',false);
        $("#SaveFinnance").trigger('reset');
    });
    ...
    $('.edit').click(function(e){
        ...
        $.ajax({
            type: "post",
            url: url+"/ProfileFinanceGet",
            data: {'id' : id},
            // dataType: "dataType",
            success: function (response) {
                $("input[name='financecode']").val(response.FinanceCode);
                $("input[name='financecode']").attr('readonly',true);
                $("input[name='financename']").val(response.FinanceName);
                $("input[name='financetel']").val(response.FinanceTel);
                $("input[name='financeaddr']").val(response.FinanceAddr);
                $("input[name='financeremark']").val(response.FinanceRemark);
                $('#type').val('1');
                $('#AddGroup').modal('show');
            }
        });
    ...
    $('#SaveFinnance').submit(function (e) { 
        e.preventDefault();
