                ...
                <div class="page-header">
                    <nav class="breadcrumb-one" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>กำหนดประเภทการจ่ายเพิ่ม</span></li>
                        </ol>
                    </nav>
                </div>
            </li>
                                ...
                                <form id="FormAddGroup" onSubmit="return false">
                                    <div class="form-group row mt-4">
                                        <div class="col-2 mt-1">
                                            <button type="button" class="btn btn-outline-primary" id="Add">เพิ่มกำหนดประเภทจ่ายเพิ่ม</button>
                                        </div>
                                    </div>
                                </form>
                                <div class="table-responsive" style="height: 650px;">
                                    <table class="table table-bordered mb-4" >
                                        <thead style="background: #ddeb60">
                                            <tr>
                                               <th>ชื่อประเภทรายรับ</th>
                                               <th>สถานะใช้งาน</th>
                                               <th>สถานะ/คำนวณภาษี</th>
                                               <th class="text-center">แก้ไข/ลบ</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach ($Reduct as $item)
                                            <tr>
                                                <td>{{ $item->ExpTypeDesc }}</td>
                                                <td>
                                                    @php
                                                        $CheckExpType = "";
                                                        if($item->ExpTypeUse == "Y"){
                                                            $CheckExpType = "Checked";
                                                        }

                                                        $CheckCalVat = "";
                                                        if($item->CalculateVAT_st == "Y"){
                                                            $CheckCalVat = "Checked";
                                                        }
                                                    @endphp
                                                    <label class="switch s-icons s-outline  s-outline-success  mb-4 mr-2">
                                                        <input type="checkbox" {{ $CheckExpType }} class="change_st_flag"  data-id="{{ $item->ExpTypeID }}">
                                                        <span class="slider"></span>
                                                    </label>
                                                </td>
                                                <td>
                                                    <label class="switch s-icons s-outline  s-outline-success  mb-4 mr-2">
                                                        <input type="checkbox" {{ $CheckCalVat }} class="change_st_vat"  data-id="{{ $item->ExpTypeID }}">
                                                        <span class="slider"></span>
                                                    </label>
                                                </td>
                                                <td class="text-center">
                                                    <ul class="table-controls">
                                                        <li class="edit" data-id="{{ $item->ExpTypeID }}">
                                                        ...
                                                        <li class="delete" data-id="{{ $item->ExpTypeID }}">
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">เพิ่มกำหนดประเภทจ่ายเพิ่ม</h5>
                ...
                <form id="SaveInsurer" action="javascript:void(0);" >
                    <div class="form-row">
                        <div class="col-md-12 mb-4">
                            ...
                            <input type="text" class="form-control required" name="ExpTypeDesc" placeholder="ชื่อประเภท" >
                        </div>
...
<script>
    ...
    $('.edit').click(function(e){
        ...
        $.ajax({
            type: "post",
            url: url+"/ProfilePayGet",
            data: {'id' : id},
            // dataType: "dataType",
            success: function (response) {
                $("input[name='ExpTypeDesc']").val(response.ExpTypeDesc);
                $('#type').val(id);
                $('#AddGroup').modal('show');
            }
        });
    ...
    $('.change_st_vat').change(function (e) { 
        e.preventDefault();
        let id = $(this).data('id');
        let status;
        if($(this).is(':checked')){
