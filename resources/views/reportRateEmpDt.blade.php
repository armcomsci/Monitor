                ...
                <div class="page-header">
                    <nav class="breadcrumb-one" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>รายงานคะแนนคนรถ (แยกตามหัวข้อ)</span></li>
                        </ol>
                    </nav>
                </div>
            </li>
                            ...
                            <div id="mailbox-inbox" class="accordion mailbox-inbox p-3">
                                <form id="FindRateEmpDriv" onSubmit="return false">
                                    <div class="form-group row mt-4">
                                        ...
                                        <div class="col-1">
                                            <select class="form-control" name="CarTypeCode" id="CarTypeCode" >
                                        ...
                                        <div class="col-3">
                                            <select class="form-control" name="TitleRate[]" id="TitleRate" >
                                                <option></option>
                                                @foreach ($RateTitle as $titleRate)
                                                    <option value="{{ $titleRate->id }}">{{ $titleRate->Title }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-3" style="display: none;">
                                            <select class="form-control" name="SubTitleRate[]" id="SubTitleRate" multiple>
                                                {{-- @if ($SubRateTitle != "")
                                                    @foreach ($SubRateTitle as $SubTitle)
    ...
    $(document).ready(function () {
        $('#Find').click(function (e) { 
            ...
            if(TitleRate == ""){
                ...
                $.ajax({
                    type: "post",
                    url: url+"/FindRateEmpDrivTitle",
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    data: $('#FindRateEmpDriv').serialize(),
                    beforeSend:function(){
                        $('.loaddingModal').css('display','block');
                        $('#DataRateTitleEmpDriv').empty();
                    },
                    success: function (response) {
                        $('.loaddingModal').css('display','none');
                        $('#DataRateTitleEmpDriv').html(response); 
                        $('#TableRateEmpTitle').dataTable({
        ...
        $('#CarTypeCode,#groupCode').change(function (e) { 
            e.preventDefault();
            $.ajax({
                type: "post",
                url: url+"/GetTitleRate",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                data: $('#FindRateEmpDriv').serialize(),
                // dataType: "dataType",
                success: function (response) {

                    let optionTitle = "<option></option>";
                    let optionSubTitle = "<option></option>";

                    $.each(response.title, function (index, value) {  
                       optionTitle += "<option value='"+value.id+"'>"+value.Title+"</option>";
                    });

                    $.each(response.Sub, function (index2, value2) {  
                        optionSubTitle += "<option value='"+value2.id+"'>"+value2.Title+"</option>";
                    });
                    ...
                    $('#TitleRate').append(optionTitle);
                    $('#SubTitleRate').append(optionSubTitle);
        ...
        $('#TitleRate').change(function (e) { 
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('#SubTitleRate').parent().css('display','block');
                    $('#SubTitleRate').empty();
                    $('#SubTitleRate').append(optionSubTitle);
                    $('#SubTitleRate').select2({
                        tags: true,
